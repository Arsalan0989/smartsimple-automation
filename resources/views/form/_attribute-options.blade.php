<div class="row mt-2">
    <div class="col-md-4 border-right h80-auto">
        ...
        {{-- <form method="post" action="{{ route('entity.alot.attribute') }}" class="frm-alot-property" onsubmit="return processTheChildForm(this)">
            @csrf --}}
            <input type="hidden" name="entityId" value="{{ $custome_form }}">
    ...
    <div class="col-md-8 h80-auto">
        <h5 class="text-primary">Alloted properties</h5>
        <span class="text-danger">Enter the property name and select the property type on left to alot new property to entity: <strong></strong></span>
        <div class="row attribListings">
            @if(!empty($custome_form) && $custome_form->attributes->count() > 0)
                @foreach($custome_form->attributes as $attribute)
